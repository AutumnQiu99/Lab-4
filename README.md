...
You work on a team that automatically flags tweets. A sentiment model is **already running in production** — we call it the **baseline**. Two newer models are proposed as replacements: **candidate_v1** and **candidate_v2**.

Your job: decide whether either candidate should **ship**. "Ship" means we would turn off the baseline and use the candidate instead.

The tempting shortcut is to compare one accuracy number and pick the winner. This lab is about why that shortcut gets people hurt, and what to do instead.

## Key ideas (read this first)

You only need five concepts. Everything in the lab is one of these.

**1. Slice.** A subgroup of the data that shares a property you care about — tweets with negation ("not", "never", "isn't"), tweets with hashtags, very long tweets, tweets with lots of emoji. A model can score 70% overall and 30% on one slice. The average hides the hole.

**2. Regression.** The baseline got a tweet **right** and the candidate gets it **wrong**. This is different from "the candidate is less accurate." A regression is a specific thing that used to work and now doesn't. Users notice regressions.
...
**4. Threshold / gate.** A rule written down *in advance*: "accuracy on the negation slice must be at least 0.60." A **gate** is a set of these rules that a model must pass. The rule is a promise about product quality, not a score you adjust until the model passes.

**5. p50 latency.** The median time to score one tweet. If p50 is 30 ms, half of tweets take longer than 30 ms. We care because this model runs on a **live moderation stream**, so being slow is a real failure even when accuracy is perfect. Our product budget is **50 ms per tweet on one CPU thread**.

Every model's timings are already measured and stored in `tweets.csv` (`{model}_latency_ms`), taken on one machine, one thread, one tweet at a time. Pytest reads those saved numbers, so your laptop's speed does not change the result.

**Exploration vs enforcement** is the arc of the whole lab. Steps 1–7 are exploration: you poke at the data and *find* problems. Step 8 is enforcement: you turn what you found into tests that run automatically on the next model.

## The three models

All three have already been run over the same 500 tweets, and their predictions are saved in `tweets.csv`. You do **not** need to run the models yourself for Steps 1–6 (no GPU, no waiting, no network).

| Name | Role | Size | Column prefix in `tweets.csv` |
|---|---|---|---|
...
Each prefix has three columns: the prediction, `_score` (the model's confidence, 0 to 1), and `_latency_ms` (time to score that tweet). `label` is the correct answer: `negative`, `neutral`, or `positive`.
...
**Keep `INCLUDE_CANDIDATE_V2 = False` until Task G.** This is deliberate. You will write your test thresholds while looking only at the baseline and v1. Then v2 has to pass rules you wrote before you ever saw its scores. That is how it works on a real team, and it stops you from quietly moving the goalposts.

## Learning objectives

After this lab you should be able to:

1. Explain why one overall accuracy number can hide a serious failure.
2. Split data into slices based on a hypothesis, and say *why* each slice might break a model.
3. Compare a candidate to a baseline using slice accuracy and regressions, not just accuracy.
4. Use W&B tables and charts to find and inspect failing examples.
5. Use an LLM to write test cases that target a weakness you already suspect.
6. Turn slices into automated tests with thresholds fixed in advance.
7. Make a ship / no-ship call, with evidence, under both a quality and a speed constraint.

## Deliverables

Show the TA:

1. **5 slices** in `slices.py` you can justify, plus your notes in `saved_slice_notes`.
2. **A W&B run** with `df_long`, `slice_metrics`, `regression_metrics`, `df_eval`, and one slice chart. Would you ship `candidate_v1`?
3. **10 LLM-generated tweets** targeting one weak slice (Step 7), and what they showed.
4. **A frozen gate:** your `tests/manifest.yaml`, plus pytest output for `MODEL=baseline` and `MODEL=candidate_v1`.
5. **A five-sentence v2 memo:** run the same gate on `candidate_v2` and recommend ship / no-ship with one mitigation.
...
Create a free W&B account at [wandb.ai](https://wandb.ai) with your CMU email, copy your key from [wandb.ai/authorize](https://wandb.ai/authorize), then in a **terminal** (not the notebook):
...
| `lab4.ipynb` | **Yes — the `#TODO` cells** | The lab itself, Steps 1–8. The four spots you fill in are listed below. |
...
### What you fill in inside `lab4.ipynb`

Every spot is marked `#TODO`. There are four:
...
4. **Steps 4.5–5.5 — the cells marked "Replace with your metadata" / "Edit to work for your slices"**: usually nothing to do. They already read `META_COLS` and `get_slices()` from `slices.py`, so keeping those current is normally enough. Touch them only if a slice of yours needs a column the pivots don't carry.
...
- [ ] Install the packages, confirm Python ≥ 3.10.
...
- [ ] Define **at least 5 slices** in `get_slices()`. For each one, write down the hypothesis: *what about these tweets might confuse a model?*
- [ ] Keep `META_COLS` in sync (the notebook checks you have at least 5).
- [ ] Re-run Step 3.

**Aim for slices with at least 30 tweets.** A slice of 4 tweets can swing from 25% to 100% by luck, so it cannot support a decision. The gate in Step 8 skips anything smaller than 30 for exactly this reason.
...
- [ ] Look at overall accuracy for both models.
- [ ] Compute accuracy **per slice**, per model.
- [ ] Compute regressions vs the baseline: regressed, improved, both wrong, both right, and confident regressions.

The interesting question is not "which number is bigger." It is: *where* does the candidate lose, and does that place matter for moderation?
...
W&B panel docs: https://docs.wandb.ai/guides/app/features/panels/

### Task E — Stress test with an LLM (Step 7)

Slicing can only find failures on tweets you happen to have. If you suspect a weakness, you can **write new tweets on purpose** to test it. That is what this step does.
